        ...
        private async void FormDetail_Load(object sender, EventArgs e)
        {
            ...
            switch (profileTypeDefinition)
            {
                case ProfileTypeDefinition.Local:
                    var profileDetails = await registry.GetRegistryPathValueAsync(profile.LocalRootKey, RegistryHive.CurrentUser);
                    guiElements.UpdateDataGridPathValue(profileDetails, this.dgProfileDetails);
                    break;
                case ProfileTypeDefinition.Citrix:
                    var citrixProfileDetails = await registry.GetRegistryPathValueAsync(profile.CitrixRootKey, RegistryHive.LocalMachine);
                    guiElements.UpdateDataGridPathValue(citrixProfileDetails, this.dgProfileDetails);
                    break;
                case ProfileTypeDefinition.FSLogix:
                    var fslogixProfileDetails = await registry.GetRegistryPathValueAsync(profile.FSLogixRootKey, RegistryHive.LocalMachine);
            ...
            var folderRefirectionDetails = await registry.GetRegistryPathValueAsync(profile.ShellFolders, RegistryHive.CurrentUser);
